"""Real outbound omni-channel dispatch (REL-010 E10.2, Notification/Omni-Channel Agent AGT-022).

Per the user's explicit platform decision for this release: Telegram and Discord get a real,
live outbound send; WhatsApp stays honestly stubbed (`OmniChannelNotifySkill` in
src/agents/tools/skills.py) since no live WhatsApp Business API token exists yet -- not silently
dropped, a documented gap.

Both functions are plain async `httpx` calls (no SDK) matching every broker adapter's own
established style (`src/brokers/kite_connect_adapter.py`, `upstox_adapter.py`). Real Telegram
Bot API / Discord API endpoints, not guessed -- confirmed against each platform's current docs.
"""
