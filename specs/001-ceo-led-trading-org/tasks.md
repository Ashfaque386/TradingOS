...
- [X] T029 [US1] Wire the scheduler: `src/agents/scheduler.py` daily research trigger calls `run_manager.create_run(objective="prepare tomorrow's trading research", source="schedule")` instead of `trigger_research()` directly (FR-001); keep `trigger_research()` importable for the composite task in US2
  - **Unblocked and completed** (Polish phase, alongside T113): the US2 task engine has existed since Phase 4, and T113's real market/news/sentiment/portfolio handlers mean a scheduled org run now does genuinely delegated work rather than stopping at a bare plan. `run_daily_research_cycle` now calls `create_run(objective=..., source="schedule")`; `trigger_research` remains a live, tested route (`POST /agents/trigger-research`) for the manual single-thread path.
...
- [X] T051 [P] [US3] Cypress `frontend/cypress/e2e/approval_gate.cy.ts`: approvals queue renders pending items; `PortfolioManager` approve moves the card and the strategy state; `ReadOnlyAuditor`/`RiskManager` see no approve/reject controls
  - **Unblocked and completed** (Polish phase): US5's console `ApprovalQueue` (`panels.tsx`) rendered approve/reject unconditionally for anyone who could view the console — the API enforced the role gate (a 403 would surface inline) but the controls themselves were always visible, unlike this task's "sees no approve/reject controls" wording. Added `manageOrgApprovals` (SA/PortfolioManager, matching `approvals.py`'s real `_can_decide` gate) to `permissions.ts` and wrapped the controls in `<Gated permission="manageOrgApprovals">` so an ineligible role now sees nothing, matching the same fully-hidden convention `Gated` already established for other mutating controls (kill switch, HITL panel). Cypress spec follows `hitl_panel.cy.ts`'s real-signup-per-role + conditional-on-live-state convention (needs the full-stack container to run); `tsc --noEmit` + `eslint` clean.
...
- [X] T085 [US7] Implement `src/orchestration/freshness.py`: wraps `src/data/datalake/freshness.py::check_freshness()`; upserts a `DatasetFreshnessRecord` per dataset (`ohlcv_daily`, `instrument_master`, `corporate_actions`, `news`, `nse_index_ohlcv`) with `last_successful_update`, `last_checksum_ok`, `status ∈ {fresh, stale, unavailable, failed}`, `retry_state` (data-model.md §11); `is_fresh(dataset)` for the planner
- [X] T086 [US7] Add a real nightly EOD ingestion job to `src/agents/scheduler.py` using the existing bhavcopy provider (REL-091…094): cadence "nightly after NSE close on trading days" (holiday-aware via `nse_holiday_calendar`), validates a checksum before setting `status = fresh` (FR-064); a failed/corrupt run sets `failed`/`unavailable` and emits `dataset.ingestion_failed` (research R10)
- [X] T087 [US7] `planner.py` / `agent_invoker.dispatch`: check `freshness.is_fresh(dataset)` for a task's `required_datasets` before dispatch; stale/unavailable → task `blocked` with `blocked_reason = "data stale: <dataset>"` while independent tasks proceed (FR-062); **never** substitute synthetic data (FR-063)
- [X] T088 [US7] Emit `dataset.stale` / `dataset.refreshed` / `dataset.ingestion_failed` events and raise an alert via the existing Grafana/Slack contact point on stale/failed (FR-062); surface per-dataset freshness in `GET /organization/runs/{id}` and `GET /attention`
- [X] T089 [P] [US7] Frontend: data-freshness panel in `OrganizationOverview.tsx` (last successful update + status per dataset)
- [X] T090 [P] [US7] Integration test `tests/integration/test_org_data_freshness.py` (quickstart Scenario 7, SC-006): stale → dependent `blocked` "data stale", independents run, `dataset.stale` event + alert; valid ingestion → unblocks; assert no synthetic rows written on this path
- [X] T091 [P] [US7] Unit test `tests/unit/test_eod_ingestion_checksum.py`: a corrupt/failed ingestion does **not** set `status = fresh` (FR-064)

**US7 delivery notes:**
- **T086**: extended the two *existing* real, already-scheduled ingestion jobs (`run_market_data_ingestion` → `ohlcv_daily`, `run_corporate_actions_ingestion` → `corporate_actions`) to call `freshness.record_ingestion_result` rather than adding a third parallel job. "Checksum" is a real structural check (zero per-symbol failures / a successful write), not a vendor-supplied hash — bhavcopy has none; this is the same honest-tradeoff pattern the rest of this codebase uses. `instrument_master` / `news` / `nse_index_ohlcv` have registered freshness rules but no ingestion job wired to report into them yet (no such job exists in this codebase) — they read `unavailable` until one does.
- **T088**: `dataset.stale`/`dataset.refreshed`/`dataset.ingestion_failed` transitions are logged (structlog) and best-effort alerted via the existing `send_ops_alert` (Telegram/Discord/Slack/PagerDuty fan-out) — **not** emitted as `OrganizationalEvent` rows, since freshness is dataset-level and `events.emit` is run-scoped. A task actually **blocked** by staleness *does* emit a run-scoped, audited `task.blocked` event naming the dataset. `GET /organization/runs/{id}` gained `dataset_freshness` (this run's required datasets); `GET /organization/attention` gained `stale_datasets`; new `GET /organization/freshness` lists every tracked dataset.
- **T090** "valid ingestion unblocks": a `blocked` task itself does not auto-resume within the same run (no retry-on-external-event mechanism exists yet); the test instead proves the real mechanism — after a checksum-passing `record_ingestion_result`, `is_fresh` flips true and a **freshly dispatched** task with the same requirement is no longer blocked by it.
...
- [X] T092 [US8] Implement an inbound-message classifier in `src/api/routers/chat.py` and `src/api/routers/webhooks.py`: actionable objective → `run_manager.create_run(objective, source=<channel>, requested_by=<mapped identity>)`, reply = the run's `CeoSynthesis` artefact (FR-130/132); pure data lookup → answer directly, **no run** (FR-133)
- [X] T093 [US8] Add an `AdHocAnalysis` capability/plan template in `planner.py` for portfolio-analysis / risk-ranking / drawdown-explanation / strategy-failure-explanation / rerun-decision objectives — assigns the appropriate existing agents + tools; result is an `AdHocAnalysis` artefact with full provenance (FR-130) and an `AuditLog` entry
- [X] T094 [US8] Channel identity: actionable objectives require the sender mapped to an authorised identity via `NOTIFICATION_CHANNELS` (SEC-030); unmapped senders are recorded (`WebhookEvent`) and **not** run (spec A-14)
- [X] T095 [P] [US8] Integration test `tests/integration/test_org_adhoc_and_omnichannel.py` (quickstart Scenario 12, SC-022): web "analyse my portfolio…" → run + real data + traceable answer + audit; channel "run today's research" → real run + synthesis reply; "what's my P&L" → direct, no run; unmapped sender → recorded, not run

**US8 delivery notes:**
- New `adhoc_classifier` prompt (PMPT-050, DB-first via `get_active_prompt`, file-fallback) classifies every inbound chat/channel message. Classification and routing now run **only** in the background thread (`src/orchestration/adhoc.py::classify_and_dispatch`, dispatched via `start_classify_and_dispatch`) — never on the request-handling thread, since a webhook (Discord in particular) must ack within a few seconds and the classifier is a real LLM call. `chat.py::send_message` and `webhooks.py::_record_event_and_route` both dispatch through this single entry point.
- The reply for a routed objective is the run's `CeoSynthesis` **or** `AdHocAnalysis` artefact (whichever the plan produced), composed once the run reaches a terminal state (`watch_run_and_reply`, 180s budget) — not literally always `CeoSynthesis` as originally phrased, since an ad-hoc analysis plan's terminal artefact is `AdHocAnalysis`.
- `planner.ensure_adhoc_scaffold` triggers on the 5 ad-hoc classifications and adds a `portfolio_read` task (if absent) feeding a terminal `adhoc_synthesis` task (`ceo_agent`, real `complete()` call in `agent_invoker._adhoc_synthesis_handler`) — mirrors T053's research scaffold pattern.
- T094 (unmapped-sender recording) was already real (`_resolve_sender` / SEC-030, pre-existing); this phase completes the identity chain by threading the resolved sender's email through as `OrganizationRun.requested_by` for Telegram/Discord/Slack. The Web chat endpoint has no auth dependency at all (pre-existing, out of scope for this phase) — its `requested_by` is `None`, not a fabricated identity.
- Fixed a real regression this scaffold caused in `test_org_planner.py::test_two_different_objectives_produce_distinct_plans` (its `objective_classification` fixture happened to be the string `"portfolio_analysis"`, coincidentally now a real US8 trigger) by renaming that fixture's classification — the test's own intent (two objectives -> two distinct, differently-sized plans) is unchanged.
...
- [X] T097 [US9] Implement the CEO unavailable-capability policy in `planner.py` / `agent_invoker.py`: on `AgentUnavailable`, the CEO chooses approved fallback / wait / reassign / escalate per config and records an `OrganizationalDecision`; the disabled agent is never dispatched (FR-005, FR-017, SC-013); queued tasks for a newly-disabled agent follow the same policy, none silently lost (FR-120)
- [X] T098 [US9] `PUT /agents/control/{name}` emits `agent.disabled` / `agent.enabled` events + `AuditLog` (actor, reason); keep the `audit_agent` disable refusal (FR-121/122); console reflects real state (already wired via T062/T068)
- [X] T099 [P] [US9] Integration test `tests/integration/test_org_agent_control.py` (quickstart Scenario 11, SC-013): disable a required agent → CEO decision recorded, agent not dispatched; re-enable → used; `audit_agent` disable → refused; events + audit present

**US9 delivery notes -- all 24 KNOWN_AGENTS now `enforced=True` (spec's own P3 checkpoint: "agent control is real for every agent"):**
- Real call sites added this phase: `execution_agent` -> `ExecutionAgent.execute_signal` (raises `ExecutionAgentDisabled` before ever calling the broker; the live-order pipeline has no production caller yet per constitution IV, but the gate is real and in place for when one exists); `notification_agent` -> `webhooks.py`'s outbound `notify_omni_channel` dispatch; `skill_registry_manager_agent` -> every skill-registry mutation endpoint (grant/revoke/enable/disable/delete) in `skills.py`; `scheduler_agent` -> `scheduler.py`'s `_tracked`/`_tracked_async` (a real whole-scheduler kill switch, on top of each job's own agent check); `ceo_agent_chat` -> `chat.py::generate_and_store_reply`. `portfolio_manager_agent` was already enforced via `agent_invoker.dispatch`'s generic gate. `audit_agent` can never actually be disabled (`set_agent_enabled`'s hard refusal, unchanged) -- its "enforcement" is that refusal itself.
- T097's CEO policy implements 2 of the 4 named responses for a disabled/unknown assigned agent: **reassign** (an enabled fallback agent declaring the same capability exists -> `task_engine._handle_agent_unavailable` reassigns and re-readies the task, recording a `reassign` decision) and **escalate** (no fallback exists -> the task is `blocked`, propagated to its dependents via the existing `mark_unsatisfiable`, and an `escalate_human` decision is recorded). No two real agents currently declare the same capability, so the reassign branch is proven with `find_by_capability` patched in `test_org_agent_control.py`; the escalate branch runs against real registry state. **Wait** and a fully **config-driven** policy selection are not implemented -- a disabled agent is an explicit, durable admin action, not a transient unavailability a bounded wait would resolve.
- T098: `agent.disabled`/`agent.enabled` are real structured log events (not `OrganizationalEvent` rows -- agent control is global, not per-run, matching the freshness precedent from US7); the existing `AGENT_CONTROL_STATE_CHANGED` audit action is unchanged and remains the durable record.
...
- [X] T100 [P] BUG-D: implement `fetch_global_indices` in `src/agents/tools/skills.py` (yfinance direct: `^GSPC`, `^IXIC`, `^N225`, `CL=F`, `INR=X` — the `IndiaVixSkill` pattern) and `query_news_sentiment` (query the `news_sentiment` Qdrant collection); leave `query_macro_calendar` / SEBI feed honestly `SkillNotImplementedError` with the coverage/governance note surfaced (FR-070/071, research R18)
  - **Note**: `query_news_sentiment` was already real (real Qdrant semantic search, done during REL-010) — only `fetch_global_indices` needed implementing. Added `market_pulse.fetch_global_index_history`/`GLOBAL_INDEX_TICKERS` (same yfinance pattern as `fetch_india_vix_history`) and made `GlobalIndicesSkill` real: fetches all 5 tickers, degrades per-ticker on failure, raises `SkillNotImplementedError` only if all 5 fail. `query_macro_calendar` stays honestly stubbed as specified.
- [X] T101 [P] BUG-F: persist an `Order` row (and publish to the REL-061 order-status stream) from `src/engine/live/execution_pipeline.py`'s execution step so live-path orders reconcile like the manual path (research "Open items", FR-162); add `tests/integration/test_live_pipeline_order_persistence.py`
  - **Note**: neither `LiveExecutionPipeline` nor `ExecutionAgent` had a production caller yet (both test-only infrastructure) — reused `ExecutionAgent` (already real: `is_agent_enabled` gate, retry-with-backoff, `Order` persistence + audit) from `handle_tick` instead of duplicating its persistence logic, closing the "no US9 gate on this pipeline" gap as a side effect. Added the REL-061 Redis publish (previously missing even from `ExecutionAgent`) to `ExecutionAgent._create_order_row` itself so every caller gets it, not just this one. `account_id`/`strategy_id` are optional (`None` default, matching `latency_guard`'s own precedent) so `handle_tick` stays pure signal-routing with no DB dependency for the 6 existing test files that don't need persistence; persistence only engages when both are set. New `tests/integration/test_execution_pipeline_persistence.py` (file name adjusted from the task's suggested name to match this module's existing test-naming convention) covers both branches.
- [X] T102 [P] BUG-H: correct stale docstrings — `src/agents/nodes/memory_agent.py` ("no scheduler exists yet") and `src/api/routers/agents.py` approve/reject ("no such feature exists in this pipeline")
  - **Note**: `agents.py`'s approve/reject docstring was already corrected as a side effect of T046 (US3) removing the no-op handlers — only `memory_agent.py`'s "no scheduler exists yet" line (stale since REL-081's real `run_weekend_memory_consolidation` job) needed fixing here.
...
- [X] T108 [P] Add per-task `timeout_seconds` enforcement in `task_engine.py` (default 300; 1800 for the `strategy_research` composite task) and the `stalled` run flag after `ORG_RUN_STALL_SECONDS` with an attention-queue entry, not auto-fail (research R20)
  - **Note**: `planner.py` now sets `timeout_seconds` per capability (1800 for `strategy_research`, else the configured default) instead of a flat hardcoded 300. `_execute_task` bounds `agent_invoker.dispatch` with `future.result(timeout=...)` in its own single-worker executor (own DB session, never shared across threads); a timeout marks the task `failed` (skips the retry ladder, same rationale as `DataStaleError`) via a conditional `UPDATE ... WHERE status='running'` so an orphaned dispatch call that finishes late can never resurrect it. `check_stalled_runs()` (new, wired as the `scheduler_stalled_run_sweep` cron job, every 5 minutes) flags a `waiting` run `stalled` once `ORG_RUN_STALL_SECONDS` has passed with no pending approval — the attention queue (`GET /organization/attention`, US5) already read `RunStatus.STALLED` but nothing had ever written it until now.
- [X] T113 Close the US4/US6 real-handler-binding deferral that T036 → T055 → T077 each re-deferred to "the next story" without ever landing: bind `market_analysis` to a real standalone `market_analyst_node` call, and `news_ingestion`/`sentiment_analysis` to real reads of the `news_sentiment` Qdrant collection the scheduled `run_news_sentiment_cycle` job already populates (never re-run ingestion synchronously inside a task), and `portfolio_read` to a real `portfolio_positions` query for the seeded Paper account, in `src/orchestration/agent_invoker.py::CAPABILITY_HANDLERS`; report real `news` dataset freshness from `run_news_sentiment_cycle` (`freshness.record_ingestion_result`) so the new `news_ingestion` handler's `reduced_coverage` is a real signal, not a hardcoded default; fix `_context_assembly_handler`'s `sector_strengths` key (never matched real `MarketContext.sector_rankings`, so `ResearchContext.sector_strengths` was always `{}` even before this task) by renaming the field to `sector_rankings: list[str]` in `artefact_schemas.py`
  - **Note**: identified as an orphaned gap while auditing tasks.md's remaining Polish-phase items, not from a pre-existing task ID — added and completed in the same pass. `strategy_research` (the composite capability that would run `build_graph()` synchronously inside a task) is intentionally **left as the honest placeholder** and tracked separately as **T113b**: that integration is materially bigger and safety-adjacent (constitution IV's ordering already holds unchanged inside `build_graph()` either way, but wiring pause/cancel/timeout semantics between the task engine and the graph's own detached-thread model correctly needs a dedicated, reviewed pass, not a drive-by addition here).
- [ ] T113b Wire the `strategy_research` composite capability in `agent_invoker.py` to actually run `build_graph()` (extending `_execute_graph_run` with an optional `research_context` param so T055's already-added `TradingOSGraphState.research_context` field is populated at runtime, not just structurally supported) synchronously inside its task-engine worker thread (already excluded from `CONCURRENCY_SAFE_CAPABILITIES`, so it already serialises); read back the resulting `AgentRun`/Strategy row via `graph_thread_id` for the task's result artefact; define pause/cancel/timeout interaction with the task engine's own lifecycle (T108) before merging
